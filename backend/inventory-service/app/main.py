...
from .consumers import handle_branch_created, handle_order_confirmed
...
async def lifespan(app: FastAPI) -> Any:  # type: ignore[misc]
    ...
    asyncio.create_task(
        consume_events(consumer_ch, "inventory-service.order.confirmed", ["order.confirmed"],
                       lambda p: handle_order_confirmed(p, sf, s))
    )
    asyncio.create_task(
        consume_events(consumer_ch, "inventory-service.branch.created", ["branch.created"],
                       lambda p: handle_branch_created(p, sf, s))
    )
...
def create_app() -> FastAPI:
    s = get_settings()
    app = FastAPI(
        title="Velontri Inventory Service",
