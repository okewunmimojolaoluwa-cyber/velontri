...
async def lifespan(app: FastAPI) -> Any:  # type: ignore[misc]
    ...
    configure_logging(settings.SERVICE_NAME, settings.SERVICE_VERSION, settings.ENVIRONMENT, settings.LOG_LEVEL)
    logger.info("marketplace_service_starting")

    engine = create_engine(settings.DATABASE_URL, settings.DB_POOL_SIZE, settings.DB_MAX_OVERFLOW, settings.DB_POOL_TIMEOUT, settings.DEBUG)
    ...
    mq_conn = await connect_with_backoff(settings.RABBITMQ_URL, settings.RABBITMQ_RECONNECT_DELAY, settings.RABBITMQ_RECONNECT_MAX_DELAY)
    ...
    # Consumer for order.completed → grant review eligibility
    consumer_channel = await mq_conn.channel()
    await setup_infrastructure(consumer_channel)

    async def _handle_order_completed(payload: dict) -> None:
        from shared.database import get_session
        from .service import MarketplaceService
        async with get_session(app.state.session_factory) as session:
            svc = MarketplaceService(session, app.state.redis, settings, mq_channel)
            await svc.handle_order_completed(payload)

    asyncio.create_task(
        consume_events(
            consumer_channel,
            queue_name="marketplace-service.order.completed",
            routing_keys=["order.completed"],
            handler=_handle_order_completed,
        )
    )

    logger.info("marketplace_service_ready")
    yield

    logger.info("marketplace_service_shutting_down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        description="Listings, reviews, stores, bookings for all Velontri marketplace types.",
    ...
    app.include_router(build_health_router(settings.SERVICE_NAME, settings.SERVICE_VERSION, _db_check, _redis_check, _mq_check))
