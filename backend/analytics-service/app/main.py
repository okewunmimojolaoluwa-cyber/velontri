...
from .consumers import handle_order_completed
...
async def lifespan(app: FastAPI) -> Any:  # type: ignore[misc]
    ...
    asyncio.create_task(consume_events(consumer_ch, "analytics-service.order.completed", ["order.completed"], lambda p: handle_order_completed(p, sf)))
...
def create_app() -> FastAPI:
    s = get_settings()
    app = FastAPI(title="Velontri Analytics Service", version=s.SERVICE_VERSION, docs_url="/docs" if s.ENVIRONMENT != "production" else None, redoc_url=None, openapi_url="/openapi.json" if s.ENVIRONMENT != "production" else None, lifespan=lifespan)
