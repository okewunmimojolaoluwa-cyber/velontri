...
@router.get('/users/admin/list', response_model=SuccessResponse, summary='Admin: list all users with search and pagination')
async def admin_list_users(request: Request, search: str=Query(default='', description='Search by name, email or phone'), page: int=Query(default=1, ge=1), page_size: int=Query(default=20, ge=1, le=100), payload: dict=Depends(get_current_user_payload)) -> SuccessResponse:
