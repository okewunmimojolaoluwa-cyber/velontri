...
from .consumers import handle_notification_send
from .routers.notifications import internal_router, router
...
async def lifespan(app: FastAPI) -> Any:  # type: ignore[misc]
    ...
    sf = async_sessionmaker(bind=engine, autocommit=False, autoflush=False, expire_on_commit=False)
    ...
    asyncio.create_task(consume_events(consumer_ch, "notification-service.notification.send", ["notification.send", "notification.*"], lambda p: handle_notification_send(p, sf, s)))
...
def create_app() -> FastAPI:
    s = get_settings()
    app = FastAPI(title="Velontri Notification Service", version=s.SERVICE_VERSION,
