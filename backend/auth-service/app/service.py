...
from shared.errors import AccountLockedError, AccountInactiveError, InvalidCredentialsError, NotFoundError, OTPExpiredError, OTPInvalidError, TokenInvalidError, ExternalServiceError, ForbiddenError
from shared.jwt_utils import ACCESS_TOKEN_TTL_SECONDS, REFRESH_TOKEN_TTL_SECONDS, create_access_token, create_refresh_token, verify_token
...
from .schemas import DeviceListResponse, DeviceResponse, IntrospectResponse, LoginResponse, TokenPair, TokenRefreshResponse, VerifyPhoneResponse
from .security import assert_not_locked, clear_failed_attempts, decrypt_totp_secret, encrypt_totp_secret, generate_otp, generate_refresh_token, generate_reset_token, generate_totp_secret, get_totp_uri, hash_otp, hash_password, hash_refresh_token, hash_reset_token, record_failed_attempt, verify_otp_hash, verify_password, verify_totp
...
class AuthService:
    ...
    async def register(self, email: str, phone: str, password: str, full_name: str, country_code: str) -> uuid.UUID:
        """
        ...
        user = None
        user = await repo.create_user(self.session, email=email, phone=phone, password_hash=password_hash, full_name=full_name, country_code=country_code)
        await publish_event(self.channel, routing_key='user.registered', payload={'user_id': str(user.id), 'email': email, 'full_name': full_name, 'phone': phone, 'country_code': country_code}, correlation_id=str(user.id))
    ...
    async def _resend_for_email(self, user_id: uuid.UUID, email: str, full_name: str, phone: str) -> None:
        ...
        await repo.create_otp(self.session, user_id=user_id, purpose='phone_verify', code_hash=otp_hash, expires_at=expires_at)
    ...
    async def verify_phone(self, user_id: uuid.UUID, otp_code: str) -> VerifyPhoneResponse:
        ...
        """
        otp_record = await repo.get_valid_otp(self.session, user_id=user_id, purpose='phone_verify')
        ...
        # Auto-create starter subscription for the newly verified user
        try:
        ...
        await publish_event(self.channel, routing_key='user.phone_verified', payload={'user_id': str(user_id)}, correlation_id=str(user_id))
        logger.info('phone_verified', user_id=str(user_id))
        return VerifyPhoneResponse()

    async def login(self, identifier: str, password: str, device_fingerprint: str, ip_address: str | None, user_agent: str | None) -> LoginResponse:
        """
        ...
        if not _is_active:
            raise AccountInactiveError('Account is not active. Please verify your phone number.')
        if not verify_password(password, user.password_hash):
            count = await record_failed_attempt(self.redis, user_id=str(user.id), max_attempts=self.settings.MAX_FAILED_ATTEMPTS, lockout_ttl=self.settings.LOCKOUT_TTL_SECONDS)
            try:
                await repo.record_login_history(self.session, user_id=user.id, device_fingerprint=device_fingerprint, ip_address=ip_address, success=False)
            except Exception:
                pass
        ...
        is_new_device = False
        try:
            device, is_new_device = await repo.get_or_create_device(self.session, user_id=user.id, fingerprint=device_fingerprint, ip_address=ip_address, user_agent=user_agent)
        ...
        try:
            await repo.record_login_history(self.session, user_id=user.id, device_fingerprint=device_fingerprint, ip_address=ip_address, success=True)
    ...
    async def enable_2fa(self, user_id: uuid.UUID, method: str, ip_address: str | None) -> dict[str, Any]:
        ...
        if method == 'totp':
            ...
            return {'method': 'totp', 'totp_secret': secret, 'totp_qr_url': uri, 'message': 'Scan the QR code with your authenticator app, then call /auth/2fa/verify to complete setup.'}
        else:
            otp = generate_otp()
            expires_at = datetime.now(tz=timezone.utc) + timedelta(seconds=self.settings.OTP_TTL_SECONDS)
            await repo.create_otp(self.session, user_id, '2fa', hash_otp(otp), expires_at)
    ...
    async def refresh_access_token(self, raw_refresh_token: str) -> TokenRefreshResponse:
        ...
        access_token = create_access_token(private_key_path=self.settings.JWT_PRIVATE_KEY_PATH, user_id=str(rt.user_id), roles=roles, subscription_tier=subscription_tier, ttl=ACCESS_TOKEN_TTL_SECONDS)
    ...
    async def request_password_reset(self, email: str) -> None:
        ...
        await self.redis.setex(key, self.settings.RESET_TOKEN_TTL_SECONDS, str(user.id))
        await publish_event(self.channel, routing_key='notification.send', payload={'recipient_user_id': str(user.id), 'channel': 'email', 'template': 'password_reset', 'data': {'full_name': user.full_name, 'reset_token': raw_token, 'expires_in_minutes': self.settings.RESET_TOKEN_TTL_SECONDS // 60}}, correlation_id=str(user.id))
    ...
    async def oauth_login(self, provider: str, id_token: str, device_fingerprint: str, ip_address: str | None, user_agent: str | None) -> LoginResponse:
        """
        ...
        if provider == 'google':
            ...
            info = await verify_apple_token(id_token, client_id=self.settings.APPLE_CLIENT_ID, team_id=self.settings.APPLE_TEAM_ID, key_id=self.settings.APPLE_KEY_ID, private_key_pem=self.settings.APPLE_PRIVATE_KEY)
        else:
        ...
        if user is None:
            ...
            user = await repo.create_user(self.session, email=info.email, phone=placeholder_phone, password_hash=password_hash, full_name=info.full_name or info.email.split('@')[0], country_code='NG')
            await repo.activate_user(self.session, user.id)
            await publish_event(self.channel, routing_key='user.registered', payload={'user_id': str(user.id), 'email': info.email, 'full_name': user.full_name, 'phone': placeholder_phone, 'country_code': 'NG', 'oauth_provider': provider}, correlation_id=str(user.id))
        if not user.is_active:
            await repo.activate_user(self.session, user.id)
        device, is_new = await repo.get_or_create_device(self.session, user_id=user.id, fingerprint=device_fingerprint, ip_address=ip_address, user_agent=user_agent)
        if is_new:
            await self._publish_new_device_alert(user, ip_address)
    ...
    async def introspect(self, raw_token: str) -> IntrospectResponse:
        ...
        return IntrospectResponse(user_id=uuid.UUID(payload['sub']), roles=payload.get('roles', []), subscription_tier=payload.get('subscription_tier', 'starter'), branch_ids=payload.get('branch_ids', []), expires_at=int(payload['exp']))

    async def list_devices(self, user_id: uuid.UUID) -> DeviceListResponse:
        devices = await repo.list_devices(self.session, user_id)
        return DeviceListResponse(devices=[DeviceResponse(id=d.id, fingerprint=d.fingerprint, ip_address=str(d.ip_address) if d.ip_address else None, user_agent=d.user_agent, last_seen=d.last_seen, is_trusted=d.is_trusted, created_at=d.created_at) for d in devices])
    ...
    async def _issue_token_pair(self, user: User, device_fingerprint: str) -> TokenPair:
        ...
        branch_ids: list[str] = []
        access_token = create_access_token(private_key_path=self.settings.JWT_PRIVATE_KEY_PATH, user_id=str(user.id), roles=roles, subscription_tier=subscription_tier, branch_ids=branch_ids, email=getattr(user, 'email', '') or '')
        ...
        try:
            await repo.create_refresh_token(self.session, user_id=user.id, token_hash=refresh_hash, device_fingerprint=device_fingerprint, expires_at=expires_at)
    ...
    async def _send_sms_otp(self, phone: str, otp: str) -> None:
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                resp = await client.post(f'{self.settings.NOTIFICATION_SERVICE_URL}/internal/sms', json={'phone': phone, 'message': f'Your Velontri verification code is: {otp}. Expires in 5 minutes.'})
    ...
    async def _send_email_otp(self, email: str, full_name: str, otp: str) -> None:
        ...
        plain_body = f'Hi {first_name},\n\nYour Velontri verification code is: {otp}\n\nThis code expires in {ttl_minutes} minutes.\n\nIf you did not create a Velontri account, ignore this email.\n\n-- The Velontri Team'
        html_body = f"<!DOCTYPE html><html><head><meta charset=utf-8></head><body style='margin:0;padding:0;background:#f1f5f9;font-family:sans-serif'><table width='100%' cellpadding='0' cellspacing='0' style='padding:40px 16px'><tr><td align='center'><table width='100%' style='max-width:520px;background:#fff;border-radius:20px;overflow:hidden'><tr><td style='background:linear-gradient(135deg,#4F46E5,#7C3AED);padding:32px;text-align:center'><h1 style='color:#fff;font-size:24px;font-weight:900;margin:0'>Velontri</h1><p style='color:rgba(255,255,255,.75);font-size:13px;margin:6px 0 0'>Africa&#39;s Premier Marketplace</p></td></tr><tr><td style='padding:36px 32px'><p style='font-size:16px;font-weight:700;color:#0f172a'>Hi {first_name},</p><p style='font-size:14px;color:#475569;line-height:1.7'>Use this code to verify your Velontri email.</p><div style='text-align:center;margin:32px 0'><div style='display:inline-block;background:#f8f7ff;border:2px dashed #818cf8;border-radius:16px;padding:24px 40px'><p style='font-size:11px;font-weight:800;color:#6366f1;text-transform:uppercase;letter-spacing:3px;margin:0 0 8px'>Verification Code</p><p style='font-size:42px;font-weight:900;color:#4F46E5;letter-spacing:12px;margin:0'>{otp}</p><p style='font-size:12px;color:#94a3b8;margin:10px 0 0'>Expires in {ttl_minutes} minutes</p></div></div><p style='font-size:13px;color:#94a3b8'>If you did not create a Velontri account, ignore this email.</p></td></tr><tr><td style='background:#f8fafc;padding:20px 32px;text-align:center;border-top:1px solid #e2e8f0'><p style='font-size:12px;color:#94a3b8;margin:0'>&copy; 2025 Velontri</p></td></tr></table></td></tr></table></body></html>"
            ...
            try:
                ...
                if gmail_refresh and self.settings.GOOGLE_CLIENT_ID and self.settings.GOOGLE_CLIENT_SECRET:
                    ...
                    async with httpx.AsyncClient(timeout=10.0) as client:
                        token_resp = await client.post('https://oauth2.googleapis.com/token', data={'client_id': self.settings.GOOGLE_CLIENT_ID, 'client_secret': self.settings.GOOGLE_CLIENT_SECRET, 'refresh_token': gmail_refresh, 'grant_type': 'refresh_token'})
                        token_resp.raise_for_status()
                        access_token = token_resp.json()['access_token']
                        send_resp = await client.post('https://gmail.googleapis.com/gmail/v1/users/me/messages/send', headers={'Authorization': f'Bearer {access_token}', 'Content-Type': 'application/json'}, json={'raw': raw_msg})
        ...
        if resend_key:
            try:
                async with httpx.AsyncClient(timeout=10.0) as client:
                    resp = await client.post('https://api.resend.com/emails', headers={'Authorization': f'Bearer {resend_key}', 'Content-Type': 'application/json'}, json={'from': 'Velontri <onboarding@resend.dev>', 'to': [email], 'subject': subject, 'html': html_body, 'text': plain_body})
        ...
        if sendgrid_key:
            try:
                ...
                async with httpx.AsyncClient(timeout=10.0) as client:
                    resp = await client.post('https://api.sendgrid.com/v3/mail/send', headers={'Authorization': f'Bearer {sendgrid_key}', 'Content-Type': 'application/json'}, json={'personalizations': [{'to': [{'email': email, 'name': full_name}]}], 'from': {'email': from_email, 'name': from_name}, 'subject': subject, 'content': [{'type': 'text/plain', 'value': plain_body}, {'type': 'text/html', 'value': html_body}]})
        ...
        logger.warning('email_otp_no_provider', email=email, hint='Set GMAIL_USER + GMAIL_APP_PASSWORD in backend/.env')
    ...
    async def _publish_lockout_notification(self, user: User) -> None:
        ...
        try:
            await publish_event(self.channel, routing_key='notification.send', payload={'recipient_user_id': str(user.id), 'channel': 'email', 'template': 'account_locked', 'data': {'full_name': user.full_name}}, correlation_id=str(user.id))
        except Exception:
    ...
    async def _publish_new_device_alert(self, user: User, ip_address: str | None) -> None:
        ...
        try:
            await publish_event(self.channel, routing_key='notification.send', payload={'recipient_user_id': str(user.id), 'channel': 'email', 'template': 'new_device_login', 'data': {'full_name': user.full_name, 'ip_address': ip_address or 'unknown'}}, correlation_id=str(user.id))
        except Exception:
