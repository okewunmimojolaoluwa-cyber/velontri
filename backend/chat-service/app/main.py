...
def create_app() -> FastAPI:
    settings = get_settings()
    app = FastAPI(title="Velontri Chat Service", version=settings.SERVICE_VERSION,
                  docs_url="/docs" if settings.ENVIRONMENT != "production" else None,
                  redoc_url=None, openapi_url="/openapi.json" if settings.ENVIRONMENT != "production" else None, lifespan=lifespan)
    configure_middleware(app)
    app.add_middleware(PrometheusMiddleware)
    register_error_handlers(app)
    app.include_router(chat_router, prefix="/api/v1")

    async def _db_check() -> bool: return await check_database_health(app.state.engine)
    async def _redis_check() -> bool: return await check_redis_health(app.state.redis)
