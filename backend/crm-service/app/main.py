...
async def lifespan(app: FastAPI) -> Any:  # type: ignore[misc]
    ...
    asyncio.create_task(consume_events(consumer_ch, "crm-service.order.completed", ["order.completed"], lambda p: handle_order_completed(p, sf)))
    logger.info("crm_service_ready")
    yield
...
def create_app() -> FastAPI:
    s = get_settings()
    app = FastAPI(title="Velontri CRM Service", version=s.SERVICE_VERSION,
