...
function Avatar({ name, size = 'md' }: { name: string; size?: 'sm' | 'md' }) {
  ...
  return (
    <div className={`${sz} flex flex-shrink-0 items-center justify-center rounded-full bg-indigo-100 font-bold text-indigo-700`}>
      {initials}
    </div>
  );
}
...
export default function UserMessagesPage() {
  ...
  // Mobile: null = thread list, string = open thread
  ...
  return (
    /* Full-height container that respects the dashboard shell */
        ...
        <div className="flex flex-col h-full md:hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            {threadsLoading ? (
              ...
              <ul>
                {filtered.map(thread => {
                  const displayName = thread.other_user_name || thread.other_user_id?.slice(0, 8) || 'User';
                  return (
                    <li key={thread.id}>
                      <button
                        onClick={() => setActive(thread.id)}
                        className="w-full flex items-center gap-3 px-4 py-3.5 text-left hover:bg-slate-50 transition-colors border-b border-slate-50 active:bg-slate-100"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[12px] text-slate-400 truncate mt-0.5">
                            {thread.last_message || (thread.listing_id ? 'Re: listing' : 'New conversation')}
                          </p>
                        </div>
      ...
      {active && (
        <div className="flex flex-col h-full md:hidden">
          ...
          <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-3 flex-shrink-0">
            ...
            <p className="text-[14px] font-bold text-slate-900 truncate flex-1">{otherUserName}</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
            {msgsLoading ? (
              ...
              <div className="flex items-center justify-center h-full">
                <p className="text-[13px] text-slate-400">Say hello! 👋</p>
              </div>
            ) : (
              messages.map(msg => {
                ...
                return (
                  <div key={msg.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[78%] rounded-2xl px-3.5 py-2.5 text-[14px] leading-relaxed break-words ${
          ...
          <div className="border-t border-slate-100 px-4 py-3 flex-shrink-0">
            <div className="flex items-end gap-2">
              <textarea
                ...
                rows={1}
                className="flex-1 resize-none rounded-xl border border-slate-200 px-3.5 py-2.5
                  text-[14px] text-slate-800 placeholder-slate-400 focus:border-indigo-400
                  focus:outline-none focus:ring-2 focus:ring-indigo-500/10 transition-all max-h-28 overflow-y-auto"
                style={{ minHeight: 44 }}
              />
              <button
                ...
                {filtered.map(thread => {
                  ...
                  return (
                    <li key={thread.id}>
                      ...
                      >
                        <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full text-[13px] font-bold ${
              ...
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  className="flex-1 resize-none rounded-xl border border-slate-200 px-4 py-2.5
                    text-[14px] text-slate-800 placeholder-slate-400 focus:border-indigo-400
                    focus:outline-none focus:ring-2 focus:ring-indigo-500/10 transition-all max-h-32 overflow-y-auto"
                  style={{ minHeight: 44 }}
                />
                <button
                  onClick={() => text.trim() && sendMsg()}
