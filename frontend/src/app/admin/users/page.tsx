...
import { Search, Users, BadgeCheck, ShieldOff, UserX, MessageCircle, CheckCircle, AlertCircle, X } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-[15px] font-black text-slate-900">Message {user.full_name}</p>
        ...
        {sent ? (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <p className="text-[13px] text-slate-500">{user.full_name} will see it in their inbox and notifications.</p>
...
export default function AdminUsersPage() {
  ...
  return (
    <RoleGate roles={['enterprise_admin', 'moderator', 'ops', 'super_admin']}
      fallback={
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between flex-wrap gap-4">
          ...
          <form onSubmit={e => { e.preventDefault(); setCommitted(search); setPage(1); }} className="flex items-center gap-2">
            <div className="relative">
              <Search className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
              <input placeholder="Search name, email, phone…" value={search} onChange={e => setSearch(e.target.value)}
                className="h-10 w-64 rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-[14px] text-slate-800 placeholder-slate-400 outline-none focus:border-indigo-400 transition-all" />
            </div>
            <button type="submit" className="h-10 rounded-xl bg-indigo-600 px-4 text-[13px] font-bold text-white hover:bg-indigo-700 transition-colors">Search</button>
          </form>
        </div>
        ...
        {isError ? (
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm p-5 space-y-3">
          ...
          <>
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <ul className="divide-y divide-slate-100">
                {users.map(user => (
                  <li key={user.id} className="grid grid-cols-1 lg:grid-cols-[1fr_1fr_auto_auto_auto_auto] gap-3 lg:gap-4 px-5 py-4 items-center hover:bg-slate-50 transition-colors">
                    ...
                    {/* Suspend/Restore */}
                    {user.id === currentUserId ? (
                      <span className="inline-flex items-center gap-1.5 rounded-full border border-indigo-100 bg-indigo-50 px-3 py-1 text-[11px] font-semibold text-indigo-600">You</span>
                    ) : (
                      <button onClick={() => toggle({ id: user.id, active: !user.is_active })}
                        className={`h-8 rounded-lg border px-3 text-[12px] font-semibold transition-all ${user.is_active ? 'border-red-200 bg-red-50 text-red-600 hover:bg-red-100' : 'border-emerald-200 bg-emerald-50 text-emerald-600 hover:bg-emerald-100'}`}>
            ...
            {meta && meta.total_pages > 1 && (
              <div className="flex items-center justify-center gap-2">
                <button disabled={!meta.has_prev} onClick={() => setPage(p => p - 1)} className="h-9 rounded-xl border border-slate-200 px-4 text-[13px] font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-35 disabled:cursor-not-allowed transition-all">Previous</button>
                <span className="px-3 text-[13px] text-slate-400">{meta.page} / {meta.total_pages}</span>
                <button disabled={!meta.has_next} onClick={() => setPage(p => p + 1)} className="h-9 rounded-xl border border-slate-200 px-4 text-[13px] font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-35 disabled:cursor-not-allowed transition-all">Next</button>
              </div>
            )}
          </>
        )}
