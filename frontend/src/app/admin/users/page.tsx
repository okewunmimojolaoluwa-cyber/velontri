        ...
        {sent ? (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <p className="text-[13px] text-slate-500">
              {user.full_name} will see it in their inbox and notifications.
            </p>
            <button
              onClick={onClose}
              className="mt-2 h-9 rounded-xl bg-indigo-600 px-5 text-[13px] font-bold text-white hover:bg-indigo-700"
            >
              Done
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={onClose}
                className="h-10 rounded-xl border border-slate-200 px-4 text-[13px] font-semibold
...
export default function AdminUsersPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () =>
      apiClient
        .get<ApiResponse<AdminUser[]>>('/users/admin/list', {
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-[13px] text-slate-400 mt-0.5">
              {meta?.total != null
                ? `${meta.total.toLocaleString()} registered users`
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-400">
                          {user.country_code} · {new Date(user.created_at).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {(user.roles.length ? user.roles : ['user']).map(r => (
                        <span
                          key={r}
                          className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold capitalize ${ROLE_CHIP[r] ?? 'bg-slate-100 text-slate-600 border-slate-200'}`}
                        >
                          {r.replace(/_/g, ' ')}
                        </span>
                      ))}
                    ...
                    {user.id !== currentUserId ? (
                      <button
                        ...
                        className="flex items-center gap-1.5 h-8 rounded-lg border border-indigo-200 bg-indigo-50
                          px-2.5 text-[11px] font-semibold text-indigo-600 hover:bg-indigo-100 transition-colors"
                      >
              ...
              <div className="flex items-center justify-center gap-2">
                <button
                  ...
                  className="h-9 rounded-xl border border-slate-200 px-4 text-[13px] font-medium
                    text-slate-600 hover:bg-slate-50 disabled:opacity-35 disabled:cursor-not-allowed transition-all"
                >
                  Previous
                </button>
