...
export default function AdminReviewsPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'published' | 'quarantined'>('all');

  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['admin', 'reviews', filter],
    queryFn: () =>
      apiClient.get<ApiResponse<Review[]>>('/admin/reviews', { params: { filter, page_size: 50 } })
        .then(r => r.data),
    staleTime: 30_000,
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {(['all', 'published', 'quarantined'] as const).map(s => (
      ...
      {isError ? (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-8 text-center">
          <p className="text-[14px] font-semibold text-red-700 mb-2">Failed to load reviews</p>
          <button onClick={() => refetch()} className="text-[13px] font-semibold text-red-600 hover:underline">Try again</button>
        </div>
      ) : isLoading ? (
        <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-24 rounded-2xl bg-slate-100 animate-pulse" />)}</div>
      ) : reviews.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 py-20 text-center">
          ...
          <p className="text-xs text-slate-400 mt-1">Reviews will appear here as users rate listings.</p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    ...
                    <span className={`text-xs rounded-full border px-2 py-0.5 font-semibold capitalize ${
                      r.status === 'published' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                      r.status === 'quarantined' ? 'bg-red-50 text-red-600 border-red-100' :
                      'bg-slate-100 text-slate-500 border-slate-200'
                    }`}>{r.status}</span>
                  </div>
                  <p className="text-sm text-slate-700 line-clamp-2">{r.comment || <span className="text-slate-400 italic">No comment</span>}</p>
