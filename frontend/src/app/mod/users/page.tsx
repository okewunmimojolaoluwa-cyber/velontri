...
import { Search, Users, ShieldOff, ShieldCheck, MessageCircle, CheckCircle, AlertCircle, X } from 'lucide-react';
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        ...
        {sent ? (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <button onClick={onClose} className="mt-2 h-9 rounded-xl bg-amber-500 px-5 text-[13px] font-bold text-white hover:bg-amber-600">Done</button>
          </div>
        ) : (
          <div className="space-y-3">
            <textarea
              ...
              className="w-full rounded-xl border border-slate-200 px-4 py-3 text-[14px] text-slate-800
                ...
                focus:ring-2 focus:ring-amber-500/10 resize-none"
            />
            {err && (
        ...
        <div className="flex gap-1.5">
          {(['all', 'active', 'suspended'] as const).map(s => (
            <button key={s} onClick={() => setFilter(s)}
              ...
              {s}
              {s !== 'all' && counts[s] > 0 && (
                <span className={`ml-1.5 rounded-full px-1.5 py-0.5 text-[10px] font-bold ${filter === s ? 'bg-white/20' : 'bg-slate-100'}`}>{counts[s]}</span>
              )}
            </button>
          ))}
        </div>
        <form onSubmit={e => { e.preventDefault(); setCommitted(search); }} className="flex gap-2 ml-auto">
          <div className="relative">
            <Search className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
            <input placeholder="Search name, emailâ€¦" value={search} onChange={e => setSearch(e.target.value)}
          ...
          <button type="submit" className="h-9 rounded-xl bg-amber-500 px-4 text-[12px] font-bold text-white hover:bg-amber-600 transition-colors">Search</button>
        </form>
      </div>
            ...
            {users.map(user => (
              <li key={user.id} className="grid grid-cols-1 lg:grid-cols-[1fr_1fr_auto_auto_auto_auto] gap-3 lg:gap-4 px-5 py-4 items-center hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-amber-100 text-[12px] font-bold text-amber-700">
                    {user.full_name?.charAt(0)?.toUpperCase() ?? '?'}
                  </div>
                  <div>
                    <p className="text-[14px] font-semibold text-slate-900">{user.full_name}</p>
                    <p className="text-[11px] text-slate-400">{user.country_code} Â· {new Date(user.created_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                  </div>
                </div>
                <div>
                ...
                {/* Message button â€” only for regular users */}
                ...
                <button disabled={!canActOn(user)} onClick={() => canActOn(user) && toggle({ id: user.id, active: !user.is_active })}
                  ...
                  {user.id === session.userId ? 'You' : user.roles.includes('moderator') ? 'Mod'
