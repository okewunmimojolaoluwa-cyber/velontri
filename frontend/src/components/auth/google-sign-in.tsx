 ...
 * - Configure Authorised origins: http://localhost:3000 in Google Cloud Console
 * - Configure Authorised redirect URIs: http://localhost:3000
 */
export function GoogleSignInButton({
  ...
  useEffect(() => {
    ...
    script.onerror = () => setUnsupported(true);
  ...
  // No client ID — show a clear placeholder
  if (unsupported || !clientId) {
    return (
      <button
        type="button"
        disabled
        title="Google Sign-In requires NEXT_PUBLIC_GOOGLE_CLIENT_ID in .env.local"
      ...
      >
        {/* Google G icon */}
        <svg width="18" height="18" viewBox="0 0 48 48" className="opacity-40">
        ...
        {label}
        <span className="ml-1 text-[11px] opacity-60">(configure client ID)</span>
      </button>
    );
  }
  ...
  return (
    <div className="relative w-full">
      ...
      <div
        ref={containerRef}
        className={`w-full ${loaded ? 'block' : 'invisible absolute'}`}
        style={{ minHeight: 44 }}
      />
    </div>
  );
